...
export default function ProfileEditor(props: Props) {
    ...
    return <main className={`flex flex-col ${props.className || ""}`}>
        ...
        <select className="font-mono bg-gray-950 p-2 rounded border-2 border-gray-800 hover:bg-gray-900 w-full cursor-pointer" value={ALL_LANGUAGES.indexOf(profile.preferredLanguage.toLowerCase())} onChange={(e) => { setProfile({ ...profile, preferredLanguage: ALL_LANGUAGES[e.target.value] }) }}>
